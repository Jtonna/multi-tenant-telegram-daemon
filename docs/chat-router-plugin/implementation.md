...
**Logging**: The handler extracts key fields from the message (message ID, chat ID, chat type, sender information, timestamp, text content) and logs them to the console in a structured format. The full raw message object is also logged for exploration purposes. This logging is always active regardless of mode.
...
**ingestMessage** sends a POST request to /api/messages with the InboundMessage as the JSON body. It throws an error if the response status is not OK, including the status code and response body in the error message.

**healthCheck** sends a GET request to /api/health and returns the parsed JSON response. This is available for diagnostic purposes but is not currently called by the bot's message flow.
...
The mapTelegramToInbound function accepts a grammY Context and returns an InboundMessage. It performs these field-by-field transformations:
...
- **senderName** concatenates from.first_name and from.last_name with a space, filtering out undefined values. If both are somehow absent, the result would be an empty string.
...
3. If a newline is found, it splits there (including the newline in the first chunk) and repeats on the remainder.
4. If no newline is found in the window, it hard-splits at exactly 4096 characters and repeats.
...
- **chatRouterClient tests** (10 tests) -- Test the mapTelegramToInbound function with mocked grammY Context objects. Verify that numeric IDs become strings, timestamps are converted to milliseconds, first and last names are concatenated properly, missing last names are handled, platform is set to "telegram", platformMeta contains the expected fields, and chat type is passed through correctly.
